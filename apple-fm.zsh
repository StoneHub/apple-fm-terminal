...
_apple_fm_close_request() {
  ...
  [[ -n $_APPLE_FM_FILE ]] && rm -f -- "$_APPLE_FM_FILE"; _APPLE_FM_FILE=''; _APPLE_FM_REQUEST_GEN=-1
}
_apple_fm_close_timer() {
  ...
  if (( _APPLE_FM_TIMER_PID > 0 )); then kill "$_APPLE_FM_TIMER_PID" 2>/dev/null; wait "$_APPLE_FM_TIMER_PID" 2>/dev/null; _APPLE_FM_TIMER_PID=-1; fi
}
_apple_fm_close_timeout() {
  ...
  if (( _APPLE_FM_TIMEOUT_PID > 0 )); then kill "$_APPLE_FM_TIMEOUT_PID" 2>/dev/null; wait "$_APPLE_FM_TIMEOUT_PID" 2>/dev/null; _APPLE_FM_TIMEOUT_PID=-1; fi
}
...
_apple_fm_response() {
  local fd=$1 status response bytes state explicit
  IFS= read -r -u "$fd" status || status=125
  _apple_fm_close_timeout
  ...
  response=$(<"$_APPLE_FM_FILE"); _apple_fm_close_request
  if [[ $status != 0 || -z $response ]]; then (( explicit )) && _apple_fm_message 'Apple FM is unavailable or returned no completion.'; return; fi
...
_apple_fm_request() {
  ...
  exec {fd}< <((child=''; trap '[[ -n $child ]] && kill "$child" 2>/dev/null; wait "$child" 2>/dev/null; exit 143' HUP INT TERM; print -rn -- "$prompt" | "$APPLE_FM_COMMAND" respond --model system --no-stream --greedy --instructions "$instructions" >| "$file" 2>/dev/null & child=$!; wait "$child"; print -r -- $?))
  _APPLE_FM_FD=$fd; _APPLE_FM_PID=$!; zle -F "$fd" _apple_fm_response
  exec {timeout_fd}< <(sleep "$APPLE_FM_TIMEOUT"; print -r -- "$_APPLE_FM_REQUEST_GEN")
  _APPLE_FM_TIMEOUT_FD=$timeout_fd; _APPLE_FM_TIMEOUT_PID=$!; zle -F "$timeout_fd" _apple_fm_timeout
}
_apple_fm_debounce() {
...
_apple_fm_schedule() {
  ...
  _apple_fm_close_timer; exec {fd}< <(sleep "$APPLE_FM_DEBOUNCE"; print -r -- "$_APPLE_FM_GENERATION")
  _APPLE_FM_TIMER_FD=$fd; _APPLE_FM_TIMER_PID=$!; zle -F "$fd" _apple_fm_debounce
}
...
apple-fm-enable() {
  ...
  autoload -Uz add-zle-hook-widget; add-zle-hook-widget -d line-pre-redraw _apple_fm_pre_redraw 2>/dev/null; add-zle-hook-widget line-pre-redraw _apple_fm_pre_redraw
  _APPLE_FM_ENABLED=1; _APPLE_FM_OBSERVED=$(_apple_fm_state)
}
apple-fm-disable() {
  (( _APPLE_FM_ENABLED )) || return; _apple_fm_cancel; autoload -Uz add-zle-hook-widget; add-zle-hook-widget -d line-pre-redraw _apple_fm_pre_redraw 2>/dev/null
